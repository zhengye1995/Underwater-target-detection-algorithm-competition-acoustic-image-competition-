...
class Matting(object):
    ...
    def __init__(self, bg_img_paths, key_word, matting_ratio=0.5):
        self.bg_img_paths = glob(bg_img_paths + key_word+'*.jpg')
        self.matting_ratio = matting_ratio
        assert len(self.bg_img_paths) > 0
        ...
        if matting_flag:
            ...
            for key in results.get('bbox_fields', []):
                for box in results[key]:
                    bg_img[int(box[1]): int(box[3]), int(box[0]): int(box[2]), :] = results['img'][int(box[1]): int(box[3]), int(box[0]): int(box[2]), :]
            results['img'] = bg_img
        return results
